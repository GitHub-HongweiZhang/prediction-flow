...
def build_embeddings(
        model, features, default_embedding_size, fixed_embedding_size=False):
    embeddings = OrderedDict()
    embedding_sizes = OrderedDict()

    for feature in itertools.chain(
            features.category_features, features.sequence_features):
        ...
        if feature.name != feature.embedding_name:
            ...
            if feature.embedding_size and (
                    feature.embedding_size != embedding_sizes[feature.name]):
